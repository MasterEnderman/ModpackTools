# Generation 0 (5 colors)

## ![Black](icons/black.png) Black
- **Hex:** `#1D1D21`
- **RGB:** `(29, 29, 33)`
- **Lab:** `(10.92, 1.06, -2.73)`
- **Mixed from:** _Base color_

## ![Blue](icons/blue.png) Blue
- **Hex:** `#3C44AA`
- **RGB:** `(60, 68, 170)`
- **Lab:** `(33.98, 29.61, -55.84)`
- **Mixed from:** _Base color_

## ![Red](icons/red.png) Red
- **Hex:** `#B02E26`
- **RGB:** `(176, 46, 38)`
- **Lab:** `(40.12, 51.64, 36.32)`
- **Mixed from:** _Base color_

## ![White](icons/white.png) White
- **Hex:** `#F9FFFE`
- **RGB:** `(249, 255, 254)`
- **Lab:** `(99.54, -2.14, -0.21)`
- **Mixed from:** _Base color_

## ![Yellow](icons/yellow.png) Yellow
- **Hex:** `#FED83D`
- **RGB:** `(254, 216, 61)`
- **Lab:** `(87.25, -1.68, 75.81)`
- **Mixed from:** _Base color_

# Generation 1 (10 colors)

## ![Arcane Red](icons/red+black.png) Arcane Red
- **Hex:** `#612925`
- **RGB:** `(97, 41, 37)`
- **Lab:** `(24.52, 25.08, 14.95)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Black (`#1D1D21`)

## ![Galaxea](icons/blue+black.png) Galaxea
- **Hex:** `#32355E`
- **RGB:** `(50, 53, 94)`
- **Lab:** `(23.78, 10.87, -24.91)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Black (`#1D1D21`)

## ![Gray](icons/gray.png) Gray
- **Hex:** `#474F52`
- **RGB:** `(71, 79, 82)`
- **Lab:** `(33.01, -2.48, -2.86)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Black (`#1D1D21`)

## ![Green](icons/green.png) Green
- **Hex:** `#5E7C16`
- **RGB:** `(94, 124, 22)`
- **Lab:** `(48.07, -25.57, 47.61)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Blue (`#3C44AA`)

## ![Light Blue](icons/light_blue.png) Light Blue
- **Hex:** `#3AB3DA`
- **RGB:** `(58, 179, 218)`
- **Lab:** `(68.17, -20.41, -29.19)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Blue (`#3C44AA`)

## ![Orange](icons/orange.png) Orange
- **Hex:** `#F9801D`
- **RGB:** `(249, 128, 29)`
- **Lab:** `(66.26, 40.82, 67.60)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Red (`#B02E26`)

## ![Pesto di Rucola](icons/yellow+black.png) Pesto di Rucola
- **Hex:** `#71833B`
- **RGB:** `(113, 131, 59)`
- **Lab:** `(51.90, -18.26, 36.28)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Black (`#1D1D21`)

## ![Pink](icons/pink.png) Pink
- **Hex:** `#F38BAA`
- **RGB:** `(243, 139, 170)`
- **Lab:** `(69.77, 43.01, 1.14)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Red (`#B02E26`)

## ![Purple](icons/purple.png) Purple
- **Hex:** `#8932B8`
- **RGB:** `(137, 50, 184)`
- **Lab:** `(39.68, 57.93, -54.52)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Red (`#B02E26`)

## ![Spätzle Yellow](icons/white+yellow.png) Spätzle Yellow
- **Hex:** `#FBEC89`
- **RGB:** `(251, 236, 137)`
- **Lab:** `(92.71, -7.99, 49.61)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Yellow (`#FED83D`)

# Generation 2 (95 colors)

## ![Apricot](icons/yellow++pink.png) Apricot
- **Hex:** `#FAB06F`
- **RGB:** `(250, 176, 111)`
- **Lab:** `(77.59, 20.24, 43.79)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Pink (`#F38BAA`)

## ![Atlantis](icons/black++light_blue.png) Atlantis
- **Hex:** `#346576`
- **RGB:** `(52, 101, 118)`
- **Lab:** `(40.16, -11.32, -14.61)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Light Blue (`#3AB3DA`)

## ![Basswood Green](icons/white+yellow++blue+black.png) Basswood Green
- **Hex:** `#81987D`
- **RGB:** `(129, 152, 125)`
- **Lab:** `(60.43, -13.49, 11.55)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Galaxea (`#32355E`)

## ![Bavarian Green](icons/yellow++blue+black.png) Bavarian Green
- **Hex:** `#779253`
- **RGB:** `(119, 146, 83)`
- **Lab:** `(57.18, -20.78, 30.27)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Galaxea (`#32355E`)

## ![Beer](icons/yellow++orange.png) Beer
- **Hex:** `#FCAB29`
- **RGB:** `(252, 171, 41)`
- **Lab:** `(76.06, 20.13, 72.12)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Orange (`#F9801D`)

## ![Berries N’ Cream](icons/white++pink.png) Berries N’ Cream
- **Hex:** `#F7BECF`
- **RGB:** `(247, 190, 207)`
- **Lab:** `(82.44, 22.98, -0.59)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Pink (`#F38BAA`)

## ![Berry Jam](icons/light_blue++red+black.png) Berry Jam
- **Hex:** `#605178`
- **RGB:** `(96, 81, 120)`
- **Lab:** `(37.35, 14.86, -19.89)`
- **Mixed from:**
  - Light Blue (`#3AB3DA`)
  - Arcane Red (`#612925`)

## ![Bimi Green](icons/gray++green.png) Bimi Green
- **Hex:** `#4E692C`
- **RGB:** `(78, 105, 44)`
- **Lab:** `(41.06, -21.08, 30.68)`
- **Mixed from:**
  - Gray (`#474F52`)
  - Green (`#5E7C16`)

## ![Brown](icons/brown.png) Brown
- **Hex:** `#835432`
- **RGB:** `(131, 84, 50)`
- **Lab:** `(40.24, 15.90, 27.69)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Orange (`#F9801D`)

## ![Bruised Plum](icons/black++red+black.png) Bruised Plum
- **Hex:** `#3F2226`
- **RGB:** `(63, 34, 38)`
- **Lab:** `(17.18, 14.49, 3.33)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Arcane Red (`#612925`)

## ![Butterum](icons/pink++yellow+black.png) Butterum
- **Hex:** `#BD8A65`
- **RGB:** `(189, 138, 101)`
- **Lab:** `(61.60, 14.89, 27.43)`
- **Mixed from:**
  - Pink (`#F38BAA`)
  - Pesto di Rucola (`#71833B`)

## ![Cactus](icons/blue++yellow+black.png) Cactus
- **Hex:** `#546F5E`
- **RGB:** `(84, 111, 94)`
- **Lab:** `(44.32, -13.71, 6.28)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Pesto di Rucola (`#71833B`)

## ![Caramel Gold](icons/yellow++purple.png) Caramel Gold
- **Hex:** `#B7956F`
- **RGB:** `(183, 149, 111)`
- **Lab:** `(63.93, 7.43, 24.87)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Purple (`#8932B8`)

## ![Celery](icons/white+yellow++green.png) Celery
- **Hex:** `#B8C33E`
- **RGB:** `(184, 195, 62)`
- **Lab:** `(75.80, -20.54, 62.38)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Green (`#5E7C16`)

## ![Classy Mauve](icons/white+yellow++purple.png) Classy Mauve
- **Hex:** `#BA969E`
- **RGB:** `(186, 150, 158)`
- **Lab:** `(65.53, 14.73, 0.91)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Purple (`#8932B8`)

## ![Coffee](icons/yellow+black++red+black.png) Coffee
- **Hex:** `#704B32`
- **RGB:** `(112, 75, 50)`
- **Lab:** `(35.41, 12.64, 21.16)`
- **Mixed from:**
  - Pesto di Rucola (`#71833B`)
  - Arcane Red (`#612925`)

## ![Cream Puff](icons/white+yellow++pink.png) Cream Puff
- **Hex:** `#F8B89A`
- **RGB:** `(248, 184, 154)`
- **Lab:** `(79.94, 19.37, 24.31)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Pink (`#F38BAA`)

## ![Cyan](icons/cyan.png) Cyan
- **Hex:** `#169C9C`
- **RGB:** `(22, 156, 156)`
- **Lab:** `(58.37, -32.24, -9.53)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Green (`#5E7C16`)

## ![Dusk](icons/blue++gray.png) Dusk
- **Hex:** `#444D77`
- **RGB:** `(68, 77, 119)`
- **Lab:** `(33.71, 8.17, -25.12)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Gray (`#474F52`)

## ![Enchanted Forest](icons/green++yellow+black.png) Enchanted Forest
- **Hex:** `#688127`
- **RGB:** `(104, 129, 39)`
- **Lab:** `(50.44, -22.79, 43.85)`
- **Mixed from:**
  - Green (`#5E7C16`)
  - Pesto di Rucola (`#71833B`)

## ![Endo](icons/light_blue++yellow+black.png) Endo
- **Hex:** `#5CA46F`
- **RGB:** `(92, 164, 111)`
- **Lab:** `(61.63, -34.48, 20.66)`
- **Mixed from:**
  - Light Blue (`#3AB3DA`)
  - Pesto di Rucola (`#71833B`)

## ![Evening Magic](icons/blue++blue+black.png) Evening Magic
- **Hex:** `#393E82`
- **RGB:** `(57, 62, 130)`
- **Lab:** `(29.23, 18.94, -39.14)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Galaxea (`#32355E`)

## ![Fading Night](icons/blue++light_blue.png) Fading Night
- **Hex:** `#3B78C3`
- **RGB:** `(59, 120, 195)`
- **Lab:** `(49.86, 5.20, -44.93)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Light Blue (`#3AB3DA`)

## ![Fizzy Peach](icons/white++orange.png) Fizzy Peach
- **Hex:** `#FAC05D`
- **RGB:** `(250, 192, 93)`
- **Lab:** `(81.19, 10.60, 56.74)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Orange (`#F9801D`)

## ![Fluorescence](icons/yellow++light_blue.png) Fluorescence
- **Hex:** `#8CD873`
- **RGB:** `(140, 216, 115)`
- **Lab:** `(79.57, -42.10, 42.34)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Light Blue (`#3AB3DA`)

## ![Forester](icons/white+yellow++gray.png) Forester
- **Hex:** `#96A472`
- **RGB:** `(150, 164, 114)`
- **Lab:** `(65.16, -13.65, 24.37)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Gray (`#474F52`)

## ![Galactic Gossamer](icons/white++blue+black.png) Galactic Gossamer
- **Hex:** `#8593BA`
- **RGB:** `(133, 147, 186)`
- **Lab:** `(61.13, 4.02, -21.92)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Galaxea (`#32355E`)

## ![Garden Weed](icons/gray++orange.png) Garden Weed
- **Hex:** `#7E6C2D`
- **RGB:** `(126, 108, 45)`
- **Lab:** `(46.11, -0.92, 37.01)`
- **Mixed from:**
  - Gray (`#474F52`)
  - Orange (`#F9801D`)

## ![Ginger Dough](icons/yellow++red+black.png) Ginger Dough
- **Hex:** `#AB7141`
- **RGB:** `(171, 113, 65)`
- **Lab:** `(52.78, 18.02, 35.61)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Arcane Red (`#612925`)

## ![Grapes of Italy](icons/gray++purple.png) Grapes of Italy
- **Hex:** `#6A4782`
- **RGB:** `(106, 71, 130)`
- **Lab:** `(36.33, 26.83, -27.56)`
- **Mixed from:**
  - Gray (`#474F52`)
  - Purple (`#8932B8`)

## ![Greenhouse](icons/green++blue+black.png) Greenhouse
- **Hex:** `#3E5D2D`
- **RGB:** `(62, 93, 45)`
- **Lab:** `(36.07, -21.27, 23.89)`
- **Mixed from:**
  - Green (`#5E7C16`)
  - Galaxea (`#32355E`)

## ![Here Comes the Sun](icons/yellow++white+yellow.png) Here Comes the Sun
- **Hex:** `#FDE25E`
- **RGB:** `(253, 226, 94)`
- **Lab:** `(89.90, -5.51, 65.95)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Spätzle Yellow (`#FBEC89`)

## ![Hinterland](icons/gray++yellow+black.png) Hinterland
- **Hex:** `#586A44`
- **RGB:** `(88, 106, 68)`
- **Lab:** `(42.46, -14.21, 19.18)`
- **Mixed from:**
  - Gray (`#474F52`)
  - Pesto di Rucola (`#71833B`)

## ![Imperial Purple](icons/blue++red+black.png) Imperial Purple
- **Hex:** `#552E62`
- **RGB:** `(85, 46, 98)`
- **Lab:** `(26.06, 27.57, -23.66)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Arcane Red (`#612925`)

## ![Indian Silk](icons/black++pink.png) Indian Silk
- **Hex:** `#845468`
- **RGB:** `(132, 84, 104)`
- **Lab:** `(41.60, 23.01, -3.18)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Pink (`#F38BAA`)

## ![Japanese Bonsai](icons/blue++white+yellow.png) Japanese Bonsai
- **Hex:** `#87A599`
- **RGB:** `(135, 165, 153)`
- **Lab:** `(65.25, -12.84, 2.79)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Spätzle Yellow (`#FBEC89`)

## ![Jovial Jade](icons/white+yellow++light_blue.png) Jovial Jade
- **Hex:** `#90DCA9`
- **RGB:** `(144, 220, 169)`
- **Lab:** `(81.83, -34.13, 17.76)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Light Blue (`#3AB3DA`)

## ![Jungle Civilization](icons/orange++blue+black.png) Jungle Civilization
- **Hex:** `#68622E`
- **RGB:** `(104, 98, 46)`
- **Lab:** `(41.02, -5.32, 30.10)`
- **Mixed from:**
  - Orange (`#F9801D`)
  - Galaxea (`#32355E`)

## ![Jungle Jewels](icons/light_blue++green.png) Jungle Jewels
- **Hex:** `#55A94E`
- **RGB:** `(85, 169, 78)`
- **Lab:** `(62.39, -43.94, 38.88)`
- **Mixed from:**
  - Light Blue (`#3AB3DA`)
  - Green (`#5E7C16`)

## ![Kiss a Frog](icons/white++yellow+black.png) Kiss a Frog
- **Hex:** `#BDC685`
- **RGB:** `(189, 198, 133)`
- **Lab:** `(77.82, -13.49, 31.61)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Pesto di Rucola (`#71833B`)

## ![Langoustine](icons/red++orange.png) Langoustine
- **Hex:** `#D44E22`
- **RGB:** `(212, 78, 34)`
- **Lab:** `(51.34, 50.81, 51.33)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Orange (`#F9801D`)

## ![Lava Stone](icons/gray++blue+black.png) Lava Stone
- **Hex:** `#3C4258`
- **RGB:** `(60, 66, 88)`
- **Lab:** `(28.28, 3.39, -13.92)`
- **Mixed from:**
  - Gray (`#474F52`)
  - Galaxea (`#32355E`)

## ![Light Gray](icons/light_gray.png) Light Gray
- **Hex:** `#9D9D97`
- **RGB:** `(157, 157, 151)`
- **Lab:** `(64.58, -1.12, 3.14)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Gray (`#474F52`)

## ![Lime](icons/lime.png) Lime
- **Hex:** `#80C71F`
- **RGB:** `(128, 199, 31)`
- **Lab:** `(73.24, -45.86, 68.31)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Green (`#5E7C16`)

## ![Lime It or Leave It](icons/light_blue++orange.png) Lime It or Leave It
- **Hex:** `#86A548`
- **RGB:** `(134, 165, 72)`
- **Lab:** `(63.71, -25.56, 44.00)`
- **Mixed from:**
  - Light Blue (`#3AB3DA`)
  - Orange (`#F9801D`)

## ![Lizard](icons/green++purple.png) Lizard
- **Hex:** `#7D6E49`
- **RGB:** `(125, 110, 73)`
- **Lab:** `(46.97, 0.21, 22.80)`
- **Mixed from:**
  - Green (`#5E7C16`)
  - Purple (`#8932B8`)

## ![Loch Ness](icons/light_blue++purple.png) Loch Ness
- **Hex:** `#656AC7`
- **RGB:** `(101, 106, 199)`
- **Lab:** `(48.51, 23.40, -49.25)`
- **Mixed from:**
  - Light Blue (`#3AB3DA`)
  - Purple (`#8932B8`)

## ![Lush Bamboo](icons/yellow++green.png) Lush Bamboo
- **Hex:** `#AFB526`
- **RGB:** `(175, 181, 38)`
- **Lab:** `(71.08, -18.49, 65.76)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Green (`#5E7C16`)

## ![Lusty Lavender](icons/blue++pink.png) Lusty Lavender
- **Hex:** `#8E5FA8`
- **RGB:** `(142, 95, 168)`
- **Lab:** `(47.96, 32.77, -31.88)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Pink (`#F38BAA`)

## ![Magenta](icons/magenta.png) Magenta
- **Hex:** `#C74EBD`
- **RGB:** `(199, 78, 189)`
- **Lab:** `(53.24, 61.52, -35.30)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Purple (`#8932B8`)

## ![Magenta Memoir](icons/pink++purple.png) Magenta Memoir
- **Hex:** `#BC5BAD`
- **RGB:** `(188, 91, 173)`
- **Lab:** `(53.16, 49.83, -26.27)`
- **Mixed from:**
  - Pink (`#F38BAA`)
  - Purple (`#8932B8`)

## ![Mauve It](icons/pink++red+black.png) Mauve It
- **Hex:** `#B04A6B`
- **RGB:** `(176, 74, 107)`
- **Lab:** `(45.89, 44.83, 2.00)`
- **Mixed from:**
  - Pink (`#F38BAA`)
  - Arcane Red (`#612925`)

## ![Midnight Express](icons/black++blue+black.png) Midnight Express
- **Hex:** `#282A3E`
- **RGB:** `(40, 42, 62)`
- **Lab:** `(17.71, 4.81, -12.99)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Galaxea (`#32355E`)

## ![Moss Gardens](icons/black++white+yellow.png) Moss Gardens
- **Hex:** `#7D8D5F`
- **RGB:** `(125, 141, 95)`
- **Lab:** `(56.32, -14.10, 22.69)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Spätzle Yellow (`#FBEC89`)

## ![Muddy Olive](icons/yellow+black++blue+black.png) Muddy Olive
- **Hex:** `#495E47`
- **RGB:** `(73, 94, 71)`
- **Lab:** `(37.62, -13.03, 10.65)`
- **Mixed from:**
  - Pesto di Rucola (`#71833B`)
  - Galaxea (`#32355E`)

## ![Muted Blue](icons/light_blue++blue+black.png) Muted Blue
- **Hex:** `#3A6F9B`
- **RGB:** `(58, 111, 155)`
- **Lab:** `(45.13, -3.61, -29.12)`
- **Mixed from:**
  - Light Blue (`#3AB3DA`)
  - Galaxea (`#32355E`)

## ![Muted Purple](icons/pink++blue+black.png) Muted Purple
- **Hex:** `#885B89`
- **RGB:** `(136, 91, 137)`
- **Lab:** `(44.97, 26.48, -18.14)`
- **Mixed from:**
  - Pink (`#F38BAA`)
  - Galaxea (`#32355E`)

## ![Mysterious Blue](icons/light_blue++gray.png) Mysterious Blue
- **Hex:** `#487E8E`
- **RGB:** `(72, 126, 142)`
- **Lab:** `(49.80, -13.33, -14.21)`
- **Mixed from:**
  - Light Blue (`#3AB3DA`)
  - Gray (`#474F52`)

## ![Never Forget](icons/pink++gray.png) Never Forget
- **Hex:** `#9B6A7D`
- **RGB:** `(155, 106, 125)`
- **Lab:** `(50.40, 22.51, -2.59)`
- **Mixed from:**
  - Pink (`#F38BAA`)
  - Gray (`#474F52`)

## ![Noble Plum](icons/purple++red+black.png) Noble Plum
- **Hex:** `#7C296C`
- **RGB:** `(124, 41, 108)`
- **Lab:** `(31.70, 43.91, -20.83)`
- **Mixed from:**
  - Purple (`#8932B8`)
  - Arcane Red (`#612925`)

## ![Not yet Caramel](icons/orange++purple.png) Not yet Caramel
- **Hex:** `#AE6E46`
- **RGB:** `(174, 110, 70)`
- **Lab:** `(52.50, 21.37, 32.64)`
- **Mixed from:**
  - Orange (`#F9801D`)
  - Purple (`#8932B8`)

## ![Over the Sky](icons/white++light_blue.png) Over the Sky
- **Hex:** `#8ED6EF`
- **RGB:** `(142, 214, 239)`
- **Lab:** `(81.88, -16.17, -19.33)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Light Blue (`#3AB3DA`)

## ![Panela](icons/red++green.png) Panela
- **Hex:** `#98501C`
- **RGB:** `(152, 80, 28)`
- **Lab:** `(41.99, 26.26, 41.92)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Green (`#5E7C16`)

## ![Panela](icons/orange++red+black.png) Panela
- **Hex:** `#9B4D1E`
- **RGB:** `(155, 77, 30)`
- **Lab:** `(41.80, 29.26, 40.97)`
- **Mixed from:**
  - Orange (`#F9801D`)
  - Arcane Red (`#612925`)

## ![Phoenix Red](icons/red++white+yellow.png) Phoenix Red
- **Hex:** `#DF7665`
- **RGB:** `(223, 118, 101)`
- **Lab:** `(61.29, 39.20, 27.84)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Spätzle Yellow (`#FBEC89`)

## ![Pinky Pickle](icons/white++red+black.png) Pinky Pickle
- **Hex:** `#B67294`
- **RGB:** `(182, 114, 148)`
- **Lab:** `(56.21, 31.66, -7.15)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Arcane Red (`#612925`)

## ![Pizza](icons/pink++green.png) Pizza
- **Hex:** `#BC8F47`
- **RGB:** `(188, 143, 71)`
- **Lab:** `(62.33, 9.11, 44.13)`
- **Mixed from:**
  - Pink (`#F38BAA`)
  - Green (`#5E7C16`)

## ![Plastic Lips](icons/red++purple.png) Plastic Lips
- **Hex:** `#9F3068`
- **RGB:** `(159, 48, 104)`
- **Lab:** `(38.70, 50.59, -7.06)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Purple (`#8932B8`)

## ![Plum Highness](icons/red++light_blue.png) Plum Highness
- **Hex:** `#845572`
- **RGB:** `(132, 85, 114)`
- **Lab:** `(42.18, 24.15, -8.47)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Light Blue (`#3AB3DA`)

## ![Potion № 9](icons/red++red+black.png) Potion № 9
- **Hex:** `#872C24`
- **RGB:** `(135, 44, 36)`
- **Lab:** `(31.99, 38.37, 26.32)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Arcane Red (`#612925`)

## ![Purple Emperor](icons/blue++purple.png) Purple Emperor
- **Hex:** `#603AB3`
- **RGB:** `(96, 58, 179)`
- **Lab:** `(35.53, 44.88, -58.53)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Purple (`#8932B8`)

## ![Purple Protégé](icons/black++purple.png) Purple Protégé
- **Hex:** `#55316C`
- **RGB:** `(85, 49, 108)`
- **Lab:** `(27.33, 28.20, -28.11)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Purple (`#8932B8`)

## ![Reading Tea Leaves](icons/yellow+black++purple.png) Reading Tea Leaves
- **Hex:** `#826968`
- **RGB:** `(130, 105, 104)`
- **Lab:** `(46.78, 9.78, 4.32)`
- **Mixed from:**
  - Pesto di Rucola (`#71833B`)
  - Purple (`#8932B8`)

## ![Rich Gold](icons/green++orange.png) Rich Gold
- **Hex:** `#A38019`
- **RGB:** `(163, 128, 25)`
- **Lab:** `(55.37, 3.92, 55.47)`
- **Mixed from:**
  - Green (`#5E7C16`)
  - Orange (`#F9801D`)

## ![Rich Gold](icons/orange++yellow+black.png) Rich Gold
- **Hex:** `#A7822A`
- **RGB:** `(167, 130, 42)`
- **Lab:** `(56.43, 5.29, 50.48)`
- **Mixed from:**
  - Orange (`#F9801D`)
  - Pesto di Rucola (`#71833B`)

## ![Ruthless Empress](icons/purple++blue+black.png) Ruthless Empress
- **Hex:** `#5B378F`
- **RGB:** `(91, 55, 143)`
- **Lab:** `(31.67, 35.49, -43.09)`
- **Mixed from:**
  - Purple (`#8932B8`)
  - Galaxea (`#32355E`)

## ![Shadow Purple](icons/blue+black++red+black.png) Shadow Purple
- **Hex:** `#492B44`
- **RGB:** `(73, 43, 68)`
- **Lab:** `(22.17, 18.43, -9.92)`
- **Mixed from:**
  - Galaxea (`#32355E`)
  - Arcane Red (`#612925`)

## ![Sinsemilla](icons/yellow++yellow+black.png) Sinsemilla
- **Hex:** `#B2B43E`
- **RGB:** `(178, 180, 62)`
- **Lab:** `(71.18, -15.54, 57.52)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Pesto di Rucola (`#71833B`)

## ![Sinsemilla](icons/white+yellow++yellow+black.png) Sinsemilla
- **Hex:** `#B9BF5B`
- **RGB:** `(185, 191, 91)`
- **Lab:** `(75.03, -16.05, 49.08)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Pesto di Rucola (`#71833B`)

## ![Siren](icons/red++blue+black.png) Siren
- **Hex:** `#693242`
- **RGB:** `(105, 50, 66)`
- **Lab:** `(28.58, 26.49, 1.49)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Galaxea (`#32355E`)

## ![Soft Boiled](icons/white+yellow++orange.png) Soft Boiled
- **Hex:** `#FBB63F`
- **RGB:** `(251, 182, 63)`
- **Lab:** `(78.65, 14.68, 66.88)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Orange (`#F9801D`)

## ![Strawberry Moon](icons/red++pink.png) Strawberry Moon
- **Hex:** `#D7536D`
- **RGB:** `(215, 83, 109)`
- **Lab:** `(53.75, 53.88, 12.83)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Pink (`#F38BAA`)

## ![Toasted Husk](icons/pink++orange.png) Toasted Husk
- **Hex:** `#F78A4A`
- **RGB:** `(247, 138, 74)`
- **Lab:** `(68.35, 36.48, 51.35)`
- **Mixed from:**
  - Pink (`#F38BAA`)
  - Orange (`#F9801D`)

## ![Totally Broccoli](icons/yellow++gray.png) Totally Broccoli
- **Hex:** `#8C9C4D`
- **RGB:** `(140, 156, 77)`
- **Lab:** `(61.56, -18.08, 39.13)`
- **Mixed from:**
  - Yellow (`#FED83D`)
  - Gray (`#474F52`)

## ![Tree Hugger](icons/blue++orange.png) Tree Hugger
- **Hex:** `#76743B`
- **RGB:** `(118, 116, 59)`
- **Lab:** `(47.83, -7.91, 31.53)`
- **Mixed from:**
  - Blue (`#3C44AA`)
  - Orange (`#F9801D`)

## ![Treetop Cathedral](icons/black++green.png) Treetop Cathedral
- **Hex:** `#334B18`
- **RGB:** `(51, 75, 24)`
- **Lab:** `(28.91, -18.98, 26.76)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Green (`#5E7C16`)

## ![Truffle Trouble](icons/red++yellow+black.png) Truffle Trouble
- **Hex:** `#9C4F32`
- **RGB:** `(156, 79, 50)`
- **Lab:** `(42.60, 29.59, 31.35)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Pesto di Rucola (`#71833B`)

## ![Very Coffee](icons/green++red+black.png) Very Coffee
- **Hex:** `#684B1B`
- **RGB:** `(104, 75, 27)`
- **Lab:** `(34.10, 6.99, 32.34)`
- **Mixed from:**
  - Green (`#5E7C16`)
  - Arcane Red (`#612925`)

## ![Vin Cuit](icons/white+yellow++red+black.png) Vin Cuit
- **Hex:** `#B27563`
- **RGB:** `(178, 117, 99)`
- **Lab:** `(55.16, 21.84, 19.78)`
- **Mixed from:**
  - Spätzle Yellow (`#FBEC89`)
  - Arcane Red (`#612925`)

## ![Vulcan](icons/black++gray.png) Vulcan
- **Hex:** `#32363A`
- **RGB:** `(50, 54, 58)`
- **Lab:** `(22.38, -0.71, -3.09)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Gray (`#474F52`)

## ![Wild Berry](icons/red++gray.png) Wild Berry
- **Hex:** `#7B3B3C`
- **RGB:** `(123, 59, 60)`
- **Lab:** `(33.28, 27.88, 12.41)`
- **Mixed from:**
  - Red (`#B02E26`)
  - Gray (`#474F52`)

## ![Wine Tasting](icons/gray++red+black.png) Wine Tasting
- **Hex:** `#58363E`
- **RGB:** `(88, 54, 62)`
- **Lab:** `(26.91, 16.47, 1.39)`
- **Mixed from:**
  - Gray (`#474F52`)
  - Arcane Red (`#612925`)

## ![Wizard’s Brew](icons/light_blue++pink.png) Wizard’s Brew
- **Hex:** `#9890B9`
- **RGB:** `(152, 144, 185)`
- **Lab:** `(61.76, 11.49, -20.26)`
- **Mixed from:**
  - Light Blue (`#3AB3DA`)
  - Pink (`#F38BAA`)

## ![Yellow Mana](icons/white++white+yellow.png) Yellow Mana
- **Hex:** `#FAF6BE`
- **RGB:** `(250, 246, 190)`
- **Lab:** `(95.98, -7.30, 27.51)`
- **Mixed from:**
  - White (`#F9FFFE`)
  - Spätzle Yellow (`#FBEC89`)

## ![Zen Garden Olive](icons/black++yellow+black.png) Zen Garden Olive
- **Hex:** `#3F4F2F`
- **RGB:** `(63, 79, 47)`
- **Lab:** `(31.47, -12.89, 16.90)`
- **Mixed from:**
  - Black (`#1D1D21`)
  - Pesto di Rucola (`#71833B`)
